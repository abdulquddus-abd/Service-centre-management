...
def showservicetypefind():
    ...
    def finddata():
        if len(e1.get())==0:
            ...
            cur=db.cursor()
            xa=e1.get()
            sql="select serviceid,prodid,catid,name,charges,durationdays from servicetype where centerid='%s'"%(xa)
            cur.execute(sql)
            data=cur.fetchone()
            e2.delete(0,END)
            e3.delete(0,END)
    ...
    heading=Label(t,text='Find st',bg='lightpink',fg='Black',font=('arial',20))
