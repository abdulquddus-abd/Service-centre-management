...
def showservicecenterupdate():
    ...
    def updatedata():
        ...
        xe=e5.get()
        xf=e6.get()
        sql="update servicecenter set name='%s',address='%s',email='%s',phone='%s',regno='%s'where centerid='%s'"%(xb,xc,xd,xe,xf,xa)
        cur.execute(sql)
        db.commit()
        db.close()
    ...
    heading=Label(t,text='Update sc',bg='lightpink',fg='Black',font=('arial',20))
    ...
    bt2=Button(t,text='finddata',command=finddata)
