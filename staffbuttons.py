...
def showstaffupdate():
    ...
    def updatedata():
        ...
        xd=e4.get()
        xe=e5.get()
        sql="update staff set staffid='%s',name='%s',address='%s',email='%s',phone='%s' where staffid='%s'"%(xb,xc,xd,xe,xa)
        cur.execute(sql)
        db.commit()
        db.close()
        messagebox.showinfo('thanks','updated')
        e1.delete(0,END)
        e2.delete(0,END)
        e3.delete(0,END)
        e4.delete(0,END)
