...
def showcalldataupdate():
    ...
    def finddata():
        if len(e1.get())==0:
            ...
            cur=db.cursor()
            xa=e1.get()
            sql="select callno,custid,prodid,warranty,engid from calldata where callno='%s'"%(xa)
            cur.execute(sql)
            data=cur.fetchone()
            e2.delete(0,END)
            e3.delete(0,END)
    ...
    def updatedata():
        ...
        sql="update calldata set custid='%s',prodid='%s',warranty=%d,engid='%s' where callno='%s'"%(xb,xc,xd,xe,xa)
        cur.execute(sql)
        db.commit()
        db.close()
