...
def showservicetypeupdate():
    ...
    def updatedata():
        ...
        xd=e4.get()
        xe=e5.get()
        sql="update servicetype set prodid='%s',catid='%s',name='%s',charges='%s',durationdays=%d where serviceid='%s'"%(xb,xc,xd,xe,xf,xa)
        cur.execute(sql)
        db.commit()
        db.close()
    ...
    def finddata():
        if len(e1.get())==0:
            ...
            cur=db.cursor()
            xa=e1.get()
            sql="select serviceid,prodid,catid,name,charges,durationdays from servicetype where serviceid='%s'"%(xa)
            cur.execute(sql)
            data=cur.fetchone()
    ...
    heading=Label(t,text='',bg='lightpink',fg='Black',font=('arial',20))
