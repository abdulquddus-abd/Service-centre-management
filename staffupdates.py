...
def showstaffupdate():
    ...
    def finddata():
        if len(e1.get())==0:
            msg.config(text='Pls Fill Staffid')
        else:
            db=pymysql.connect(host='localhost',user='root',password='root',database='scm')
            cur=db.cursor()
            xa=e1.get()
            sql="select staffid,name,address,email,phone from staff where staffid='%s'"%(xa)
            cur.execute(sql)
            data=cur.fetchone()
    ...
    def updatedata():
        ...
        xd=e4.get()
        xe=e5.get()
        sql="update staff set name='%s',address='%s',email='%s',phone='%s' where staffid='%s'"%(xb,xc,xd,xe,xa)
        cur.execute(sql)
        db.commit()
        db.close()
    ...
    heading=Label(t,text='Update staff',bg='lightpink',fg='Black',font=('arial',20))
