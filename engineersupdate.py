...
def showengineersupdate():
    ...
    def updatedata():
        ...
        xe=e5.get()
        xf=e6.get()
        sql="update engineers set name='%s',address='%s',phone='%s',email='%s',catid='%s' where engid='%s'"%(xb,xc,xd,xe,xf,xa)
        cur.execute(sql)
        db.commit()
        db.close()
    ...
    heading=Label(t,text='Update engineers',bg='lightpink',fg='Black',font=('arial',20))
