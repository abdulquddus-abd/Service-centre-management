...
def showcallfeedbackupdate():
    ...
    def updatedata():
        ...
        if xe == "" or xe == "0":
            ...
            sql="update callfeedback set custid='%s',engid='%s',ratings=%d,remarks='%s' where callno='%s'"%(xb,xc,xd,xe,xa)
            cur.execute(sql)
            db.commit()
            db.close()
    ...
    def finddata():
        if len(e1.get())==0:
            ...
            cur=db.cursor()
            xa=e1.get()
            sql="select callno,custid,engid,ratings,remarks from callfeedback where callno='%s'"%(xa)
            cur.execute(sql)
            data=cur.fetchone()
            e2.delete(0,END)
            e3.delete(0,END)
    ...
    heading=Label(t,text='Call feedback update',bg='lightpink',fg='Black',font=('arial',20))
